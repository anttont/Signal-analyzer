...
from matplotlib.widgets import Button as py_btn

blocksize = 1e6  # Adjust the chunk size if necessary
max_y_limit = 500  # Y-axis limit
target_data_points_per_second = 5000000  # Processing rate
window_size = 500000  # Size of window
...
def callback():
    ...
    for partition_number in range(1, len(df.divisions) - 1):
        ...
        ax.text(
            0.02,
            0.85,
            f"Target DPS: {target_data_points_per_second:.2f}",
            transform=ax.transAxes,
        )
        ax.text(
            0.02, 0.80, f"Data window size: {window_size:.2f}", transform=ax.transAxes
        )
        ax.text(
            0.02,
            0.75,
            f"Peak amount: {int(len(peak_indices_unadjusted_list) / 50):.2f}",
            transform=ax.transAxes,
        )
    ...
    axs[0].text(
        0.02,
        0.75,
        f"Peak amount: {len(all_peak_indices):.2f}",
...
btn.pack(ipadx=10)
btn_start = Button(win, text="Start algorithm", command=callback)
